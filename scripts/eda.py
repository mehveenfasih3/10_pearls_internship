...
def clean_air_quality_data(df):
    ...
    for col in pollutant_cols:
        if col in df_clean.columns:
            ...
            if negative_count > 0:
                print(f"     {col}: Found {negative_count} negative values, setting to 0")
...
def validate_dataframe_for_hopsworks(df):
    ...
    if df_clean.columns.duplicated().any():
        print("  Issue: Duplicate column names detected")
    ...
    for col in df_clean.columns:
        ...
        if len(types_in_col) > 2:  # More than 2 types (including NaN)
            print(f"  Issue: Mixed types in column '{col}': {types_in_col}")
